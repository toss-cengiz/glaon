...
- **Codespaces prebuild "moby-cli not found" hatası** → Base image Debian trixie'ye geçtiğinde `moby-cli` paketi düştü. `.devcontainer/devcontainer.json`'da `docker-outside-of-docker` feature'ı `"moby": false` ile geliyor; CLI olarak upstream Docker CE'yi (`docker-ce-cli` + buildx + compose plugin) `download.docker.com`'dan kuruyor. Davranış aynı (host daemon socket'i mount, "outside of docker"); sadece CLI paket kaynağı değişti.
