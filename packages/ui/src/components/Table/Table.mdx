...
the canonical Glaon card surface — title row, optional filters bar,
the table body, optional pagination. Mirrors the Tables-page Figma
frames where every realistic table renders inside this chrome.
...
<Table.Card>
  ...
  <Table.Card.Filters>
    ...
    <Button color="secondary" iconLeading={Filter}>Filter</Button>
  </Table.Card.Filters>
...
- **`Table.Card`** (alias `Table.Card.Root`) — the rounded outer
  surface with shadow + border. Re-exports the kit `TableCard.Root`
  primitive.
- **`Table.Card.Header`** — title + description + optional `badge`
  + `actions` slot (typically primary CTA + dropdown). Re-uses the
  kit `TableCard.Header`'s `contentTrailing` slot under the
  Glaon-friendly `actions` prop name.
- **`Table.Card.Filters`** — slot for consumer-composed filter
  controls (ButtonGroup, Tabs, Select, Button mix). Flex row that
  wraps on narrow viewports.
- **`Table.Card.Pagination`** — Prev / page-count / Next paginator
  with optional per-page selector. The V1 paginator covers the
  common "page X of Y" case; Phase 2's `DataTable` swaps in a
  richer paginator with virtualisation hooks.

A11y:
- The header `<h2>` semantic is provided by the kit primitive.
- `<Table.Card.Pagination>` renders as `<nav aria-label>`; consumers
  can override the label via `ariaLabel` when the table sits next
  to multiple paginators.
- Filters bar leaves ARIA to the consumer's controls — `<ButtonGroup
  aria-label="Status filter">` etc.

Server-side pagination integration is consumer-orchestrated (wire
`page` / `pageCount` to TanStack Query state); the paginator only
emits `onPageChange` events.

See [#328](https://github.com/toss-cengiz/glaon/issues/328) for the
Phase E scope.
