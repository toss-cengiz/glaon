...
<<<<<<< 209-untitledui-env-interpolation
- The MCP entries in `.mcp.json` (Chromatic, Figma, Untitled UI) are part of the repo contract — don't edit or remove them as a drive-by. New MCP servers get their own entry and their own issue; breaking changes to existing URLs require a tracked migration PR. Auth lives per-user and is never committed: Figma MCP uses browser OAuth (see [docs/figma.md](docs/figma.md#per-developer-kurulum)); Untitled UI MCP uses `${UNTITLED_TOKEN}` env interpolation in the header — token comes from `.env` (template at `.env.example`, exported into the shell that launches Claude Code). See [packages/ui/README.md](packages/ui/README.md#untitled-ui-remote-mcp) for the Untitled UI env setup.
=======
- The MCP entries in `.mcp.json` (Chromatic, Figma, Untitled UI) are part of the repo contract — don't edit or remove them as a drive-by. New MCP servers get their own entry and their own issue; breaking changes to existing URLs require a tracked migration PR. Auth tokens for OAuth-based MCPs (Figma, Untitled UI) live per-user in Claude Code config and are never committed; see [docs/figma.md](docs/figma.md#per-developer-kurulum) for the per-developer authenticate flow (Untitled UI follows the same pattern — `/mcp` → `untitledui` → Authenticate, browser-based OAuth).
>>>>>>> development
