...
- UI: `@glaon/ui` wraps the licensed Untitled UI React kit on a Tailwind v4 + UUI `theme.css` + react-aria-components foundation (kit source pulled via the official CLI; see [ADR 0011](docs/adr/0011-untitled-ui-react-kit.md), [ADR 0013](docs/adr/0013-tailwind-v4-uui-theme.md))
...
- The MCP entries in `.mcp.json` (Chromatic, Figma, Untitled UI, Storybook) are part of the repo contract — don't edit or remove them as a drive-by. New MCP servers get their own entry and their own issue; breaking changes to existing URLs require a tracked migration PR. Auth lives per-user and is never committed: Figma MCP uses browser OAuth (see [docs/figma.md](docs/figma.md#per-developer-kurulum)); Untitled UI MCP uses `${UNTITLED_TOKEN}` env interpolation in the header — token comes from `.env` (template at `.env.example`, exported into the shell that launches Claude Code). The Storybook MCP entry points at `http://localhost:6006/mcp` and is only live while `pnpm --filter @glaon/ui storybook` is running — see [docs/storybook.md](docs/storybook.md#mcp-ai-agent-entegrasyonu). See [packages/ui/README.md](packages/ui/README.md#untitled-ui-remote-mcp) for the Untitled UI env setup.
...
- Two canonical Figma files (always reachable; pinned here so they never drift out of context):
  ...
  - **Design System** — Glaon's published primitive library. Variables + text styles + primitive components live here; primitives are designed directly in this file (no separate source library). <https://www.figma.com/design/cDLzPUkcsDJtvwqZLWRwrd/Design-System>
